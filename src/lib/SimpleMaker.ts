import { HydroClient, Side } from "@hydro-protocol/sdk";
  ...
  public async run(interval: number, amount: BigNumber) {
    ...
    while (true) {
      ...
      if (!this.lastPrice.eq(ticker.price)) {
        ...
        let orderbook = await this.client.getOrderbook(this.marketId, 1);
        ...
        let orderPriceStr = new BigNumber(
          orderPrice.toFixed(market.priceDecimals)
        ).toPrecision(market.pricePrecision);

        // Create the new order
        let order = await this.client.createOrder(
          this.marketId,
          side,
          "limit",
          orderPriceStr,
